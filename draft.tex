...
本ツールによる，対象手法のシナリオとして正しいシナリオを作成する手順を，開発者が行う操作を基に説明する．シナリオ作成は，図\ref{fig:scenarioOutline}に示す四段階に分かれる．また，これに係る内部動作は\ref{sec:}で説明する．
...
最初に，事前条件 (Precondition) を作成する．事前条件はシナリオ開始前．また，画面インスタンスの集合として表される．これを作成するには，任意の画面から画面インスタンスを作成するか，既存の画面インスタンスを再利用し，集合に追加する．具体的には，画面もしくは画面インスタンスを選択すると図\ref{fig:scenarioPrecondition}のように入力項目が表示されるので，そこに具体値を入力する．入力後，「事前条件に追加」ボタンを押下すると事前条件に追加される．

事前条件の作成に関するツール側の処理は，\ref{sec:}で説明する．

次に，想定されるユーザーの操作からシナリオ本体を作成する．本ツールにより，画面遷移モデルから，対象のシステムの画面が擬似的に再現される．これを実際に操作することで，シナリオ本体を作成していく．画面の擬似的な再現は，シナリオ内で最初に表示される画面を選択して「シナリオ作成の開始」ボタンを押下すると開始される（図\ref{fig:scenarioStart}）．
...
値の入力後，ユーザーが行うと想定される遷移を行う．下部のボタンを押下すると，遷移意図を選択するメニューが出現する（図\ref{fig:transitionSelection}）．これはどのような目的で遷移を行うかを示すもので，対象の手法に則って「作成」，「閲覧」，「更新」，「削除」，「取消」が存在する．また，本ツールには，明示する必要の無い遷移に用いる「意図なし」も存在する．
...
値の入力と遷移を繰り返し，シナリオを作成していく．



「シナリオの作成」を押下すると，対象の手法のシナリオの形式として正しいシナリオを本ツールが自動的に生成する．本ツールのシナリオ生成に関しては，\ref{sec:createScenario}章で説明する．
