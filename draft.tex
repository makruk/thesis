...
第一に，シナリオを作成する際，大量の値を入力する必要がある．シナリオには各画面で入力される値が記述される．このとき，同じ画面に対する入力には同一のものや類似しているものがあり，容易に補完できる．従って，入力された値をツールが記憶し，必要な場所で自動入力する．これにより，シナリオを作成する際の入力量を低減させる．
...
\begin{description}
  ...
  \item[\texttt{<<destination>>}]{意図の中で最後に到達する画面インスタンスであることを示す(図\ref{fig:SI_dist})．}
...
\texttt{<<delete>>}，\texttt{<<cancel>>}，\texttt{<<destination>>}は，図\ref{fig:SI_dist}のように画面インスタンス名の前に付加する．また，システムの自動入力は図\ref{fig:SI_a}のように項目の末尾に付加する．

本研究では記述の誤りを防ぐため，ツールによる拡張記法の自動補完を行う．

\begin{figure}[htbp]
  \subfigure[拡張記法なし]{\includegraphics[width=0.5\columnwidth]{img/screenInstance.eps}\label{fig:SI_normal}}
  \subfigure[意図の中で最後に到達する画面]{\includegraphics[width=0.5\columnwidth]{img/screenInstance.eps}\label{fig:SI_dist}}
  \subfigure[システムによる自動入力]{\includegraphics[width=0.5\columnwidth]{img/screenInstance.eps}\label{fig:SI_a}}
  \subfigure[複数値]{\includegraphics[width=0.5\columnwidth]{img/screenInstance.eps}\label{fig:SI_multi}}
  \caption{画面インスタンス}
...
\section{関連研究}
開発の早期に問題を発見する手法として，プロトタイプを提示する手法が多数提案されている\endnote[2]{小形真平，松浦佐江子，UML要求分析モデルからの段階的なWeb UIプロトタイプ自動生成手法，コンピュータソフトウェア，Vol.27，No.2(2010)，pp.14-32}\endnote[3]{Shinobu Saito, Jun Hagiwara, Tomoki Yagasaki, Katsuyuki Natsukawa, Requirements Model Oriented System Generation Tool and Its Practical Evaluation, Computer Software and Applications Conference Workshops (COMPSACW), 2014 IEEE 38th International, pp.535-540}．小形らの提案する手法\endnotemark[2]では，アクティビティ図，クラス図，オブジェクト図からプロトタイプを自動生成する．これにより，開発者と顧客の認識を合わせることができる．Saitoらの提案する手法\endnotemark[3]は，5つの要求モデルから，機能が付帯されたプロトタイプを生成する．これにより，要求分析の誤りを減少させる．さらに，機能付きプロトタイプを生成することで，開発にかかる時間を短縮する．

これらの手法は，要求分析段階における開発者と顧客の間の認識の差に主眼を置いており，システムの評価を行うものではない．

開発の初期段階にユーザビリティを定量的に評価する手法\endnotemark[1]が早川らによって提案されている．この手法ではユーザビリティの問題を定量的に評価できるが，手法の導入にかかるコストが大きい．そこで，本手法では，早期に問題を発見する手法の支援を行う．
...
これにより，以下の効果を期待する．第一に，拡張された記法をツールが補完し，初学者がシナリオを作成した際の文法的な誤りを防止する効果である．第二に，以前入力した値をツールがすべて記憶し，適宜挿入することでシナリオを作成する際の入力量を低減する効果である．
...
\begin{figure}[htb]
  \subfigure[対象手法のみ]{\includegraphics[width=0.5\columnwidth]{img/component_inputItem.eps}\label{fig:CI_only}}
...
最初に，事前条件 (Precondition) を作成する．事前条件はシナリオが始まる前のシステムの状態である．これは画面インスタンスの集合として表される．これを作成するには，任意の画面から画面インスタンスを作成するか，既に存在するシナリオの画面インスタンスを再利用し，集合に追加する．具体的には，画面もしくは画面インスタンスを選択すると図\ref{fig:scenarioPrecondition}のように入力項目が表示されるので，そこに具体値を入力する．入力後，「事前条件にインスタンスを追加する」ボタンを押下すると事前条件に追加される．
...
再現された画面に対して，想定される入力値を実際に入力する．この時，複数の値を入力する場合は入力項目の隣の「＋」ボタンを押下する．また，入力項目を削除する場合，隣の「×」ボタンを押下する（図\ref{fig:inputComponentAddDel}）．
...
\begin{description}
  ...
  \item[意図なし]{遷移先の画面インスタンスを変更せず，かつ明示的に遷移する必要が無い場合に行う．対象手法では明確な操作手順は記述されないため，すべての遷移を記録する必要はない．従って，シナリオ上明示する必要のない遷移はすべて意図なしになる．}
\end{description}
...
\subsection{概要}
本ツールを使用した場合のシナリオ作成への影響を測るため，以下の三点について評価を行う

\begin{itemize}
  \item 本ツールを使用して，正しいシナリオを記述できるか
  \item シナリオを記述する時間を短縮できるか
  \item 対象手法に不慣れな人もシナリオを記述できるか
\end{itemize}

\subsection{実験の内容}
最初に，対象手法，本ツールについて説明を行う．続いて，被験者にはチュートリアルとして小規模なシステムに関するシナリオを書いてもらう．その後，被験者にシステムの画面のスクリーンショットから作成した資料を見せ，シナリオを作成してもらう．
...
被験者と，行った実験について表\ref{tab:subject}に示す．

\begin{table}[htb]
  \begin{tabular}{|r||r|r|} \hline
    被験者＼実験 & 一回目 & 二回目 \\ \hline \hline
    熟練A    &ツール使用  &ツール未使用 \\  \hline
    熟練B    &ツール未使用&ツール使用 \\   \hline
...
\subsection{評価方法}
シナリオの比較方法として，以下の三点の指標から評価を行う．

\begin{description}
\item[形式の誤り] 対象手法の形式として誤ったもの．具体的には，読み込む時エラーが発生する物．
\item[文法の誤り] 読み込みは可能だが，記法に誤りがある物．具体的には，拡張記法の誤字．
\item[意味の誤り] 用意されたシナリオと意味が違うもの．
\end{description}

また，シナリオの作成にかかった時間も測定する．
...
図\ref{fig:evalScenario}に，評価に用いたシナリオを示す．

\subsection{考察・結果}
一回目の実験結果は表\ref{tab:test1}に，二回目の実験結果は表\ref{tab:test2}に示す．

一回目の実験と二回目の実験を比較すると，全体的に二回目の実験の作業時間が短い．これは同一のシナリオを利用したことが原因とみられる．

\begin{table}[htb]
  \begin{tabular}{|c|c||r|r|r|r|} \hline
    被験者 & ツールの使用 & 作業時間(秒) & 形式の誤り & 文法の誤り & 意味の誤り \\ \hline \hline
    熟練A    &○&2860&　なし & なし & なし \\ \hline
    熟練B    &×&2114& なし & なし &　5 \\ \hline
    初学者C  &○&3398& あり & なし & - \\  \hline
    初学者D  &×&3663& なし & なし & 3 \\  \hline
  \end{tabular}
  \label{tab:test1}
  \caption{一回目の実験}
\end{table}
\begin{table}[htb]
  \begin{tabular}{|c|c||r|r|r|r|} \hline
    被験者 & ツールの使用 & 作業時間(秒) & 形式の誤り & 文法の誤り & 意味の誤り \\ \hline \hline
    熟練A    &×&1696& なし & なし & 1 \\ \hline
    熟練B    &○&1457& あり & なし & なし \\  \hline
    初学者C  &×&2597& なし & なし & なし \\  \hline
    初学者D  &○&1972& あり & なし & なし \\  \hline
  \end{tabular}
  \label{tab:test2}
  \caption{二回目の実験}
\end{table}
