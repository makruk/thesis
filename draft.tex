...
遷移および意図に関する詳細は章\ref{sec:aboutHistory}で説明する．
...
本ツールのシナリオ作成の手順を，開発者が行う操作を基に説明する．シナリオ作成は，図\ref{fig:scenarioOutline}に示す四段階に分かれる．また，これに係る内部動作は\ref{sec:}で説明する．
...
シナリオの作成は，開発者が「シナリオ作成開始」ボタンを押下すると開始される（図\ref{fig:scenarioFirst}）．シナリオの作成開始時に完成した画面遷移モデルを読み取る必要があるため，開発者による操作が必要になる．

最初に，事前条件 (Precondition) を作成する．事前条件は画面インスタンスの集合である．これを作成するには，任意の画面から画面インスタンスを作成するか，既存の画面インスタンスを再利用し，集合に追加する．具体的には，画面もしくは画面インスタンスを選択すると図\ref{fig:scenarioPrecondition}のように入力項目が表示されるので，そこに具体値を入力する．入力後，「事前条件に追加」ボタンを押下すると事前条件に追加される．

事前条件の作成に関するツール側の処理は，\ref{sec:}で説明する．

次に，想定されるユーザーの操作を本ツール上で実際に行い，シナリオの主な部分を作成する．本ツールにより，画面遷移モデルから対象のシステムの画面が擬似的に再現される．これを実際に操作し，シナリオを作成する．画面の擬似的な再現は，シナリオ内で最初に表示される画面を選択して「シナリオ作成の開始」ボタンを押下すると開始される（図\ref{fig:scenarioStart}）．

再現された画面に対して，想定されるユーザーの入力値を実際に入力する．この時，複数の値を入力する場合は入力項目の隣の「＋」ボタンを押下する．また，入力項目を削除する場合，隣の「×」ボタンを押下する（図\ref{fig:inputComponentAddDel}）．

値の入力後，ユーザーが行うと想定される遷移を行う．遷移意図を選択するメニューが出現する（図\ref{fig:transitionSelection}）．これはどのような目的で遷移を行うかを示すもので，対象の手法に則って「作成」，「閲覧」，「更新」，「削除」，「取消」が存在する．また，本ツールには，明示する必要の無い遷移に用いる「意図なし」も存在する．
...
\begin{description}
  ...
  \item[意図なし]{遷移先の画面インスタンスを変更せず，かつ明示的に遷移する必要が無い場合に行う．対象手法では明確な操作手順は記述されず，すべての遷移を記録する必要もない．従って，シナリオ上明示する必要のない遷移はすべて意図なしになる．}
\end{description}
...
[label aboutHistoryの内容を含める?]
[シナリオの作成手順をこれの後にする?　あと、シナリオの作成手順にはユーザー側の操作のみを記述し、シナリオの導出に内部的な動作を含める?]
