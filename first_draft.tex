...
入力項目は入力型 (InputType) を持つ．これは入力項目の型として表される（図\ref{fig:trasitionModel}）．入力型は三種類あり，文字列入力 (InputText)，単一選択(SingleSelection)，複数選択(MultipleSelection)に分類される．

\subsection{画面インスタンス}
画面インスタンスは，画面 (Screen)をインスタンス化したもので，各画面で入力された値が格納される．

また，これには以下の六種類の拡張記法が定義されており，シナリオの作成者が適切に入力しなければならない．
