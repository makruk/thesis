...
\section{評価}
本ツールを使用した場合のシナリオ作成の難度の変化，負担の変化，効率性を測るため，本ツールを用いて実験を行った．実験の結果から，本ツールの有効性を考察した．

\subsection{評価方法}
本ツールの有効性を確認するため，ツールの使用時と未使用時で作成されたシナリオの比較，作成にかかる時間の比較を行った．また，実験後に被験者から本ツールに関する講評を受けた．

シナリオの比較方法として，以下の三点の指標から評価を行った．
...
\section{関連研究}
開発の早期段階において操作可能なUIプロトタイプを作成する手法として，UML要求分析モデルからの段階的なWeb UIプロトタイプ自動生成手法\endnote[2]{小形真平，松浦佐江子，UML要求分析モデルからの段階的なWeb UIプロトタイプ自動生成手法，コンピュータソフトウェア，Vol.27，No.2(2010)，pp.14-32}が提案されている．この手法では開発者と顧客の認識を合わせるため，アクティビティ図，クラス図，オブジェクト図からプロトタイプを生成する．ただし，これは要求分析を行うことを主眼に置いており，品質の評価を行うものではない．
