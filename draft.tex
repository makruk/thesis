...
\begin{description}
  \item[\texttt{<<delete>>}]{画面インスタンス名の接頭．画面インスタンスが削除されたことを示す．}
  \item[\texttt{<<cancel>>}]{画面インスタンス名の接頭．入力された値が取り消されたことを示す．}
  \item[\texttt{<<destination>>}]{画面インスタンス名の接頭．意図の中で最後に到達する画面インスタンスであることを示す．(図\ref{fig:SI_dist})}
  \item[\texttt{\symbol{92}}a]{入力値および画面インスタンス名の接尾．入力値がシステムによる自動入力であることを示す．(図\ref{fig:SI_a})}
  \item[\texttt{\symbol{92}},]{入力値に対する拡張記法．一つの項目に対して複数の値を入力する際に用いる．(図\ref{fig:SI_multi})}
\end{description}

正しいシナリオを作成するには，拡張記法を適切に記述する必要がある．画面インスタンス名の接頭は図\ref{fig:screenInstance}の
